...
def analyze_chunks_of_commits_in_parallel():
    ...
    num_commits = 0
    for c in Repository(url, since=begin, to=to, only_no_merge=True, clone_repo_to=os.getcwd()).traverse_commits():
        relCLOC = utils.calculateRelCLOC(os.path.join(os.getcwd(), repo_name), c, diff_exclude)
        if relCLOC == 0 or (maxCLOC is not None and relCLOC > maxCLOC):
            continue
        num_commits += 1
